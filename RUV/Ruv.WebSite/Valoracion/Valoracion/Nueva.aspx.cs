...
public partial class Valoracion_Valoracion_Nueva : PaginaBase, IFormularioGuardar
{
    ...
    protected void mpopGuardar_Ok(object sender, EventArgs e)
    {
        ...
        {
            Response.Redirect(string.Format("Resumen.aspx?ValId={0}", ObtenerValoracionActual().Id));
        }
        else
        {
            Finalizar();
    ...
    private void Replicar(clsPersonaAnexo Nper, clsPersonaAnexo persona)
    {
        foreach (var item in Nper.GetType().GetProperties())
        {
            if (item.Name.Equals("Afectado") || item.Name.Equals("Victima") || item.Name.Equals("AfectacionesDetectadas")
                || item.Name.Equals("EstadoId") || item.Name.Equals("ObservacionId") || item.Name.Equals("Principios"))
            {
    ...
    public bool Guardar(eEstadosValoracion estado, bool finalizar = false)
    {
        ...
        else { // Se marcará como devolución
            if (finalizar) {
        ...
        if (!string.IsNullOrEmpty(guardar)) {
            ...
            url = url.Substring(0, url.LastIndexOf("/")) + "/Default.aspx";
            Master.MensajeDeError.Mostrar(url);
            return false;
        }
    ...
    private void Finalizar() {
        ...
        if (valida.Count == 0) {
            if (Guardar(eEstadosValoracion.PendientePorNotificar, true)) {
                mpopGuardar.VisibleBotonCancelar = false;
                mpopGuardar.Mensaje = "Se finalizó con exito la Valoración";
                mpopGuardar.Mostrar();
            }
        }
        else {
    ...
    private ListItemCollection EsValido() {
        ...
        if ((dvBasicaInfo2.FindControl("chkValidacion") as CheckBox).Checked) {
            ...
            if ((string.IsNullOrEmpty(valoracionActual.Motivacion_Inclusion) || valoracionActual.Motivacion_Inclusion.Equals(" "))
                && (string.IsNullOrEmpty(valoracionActual.Motivacion_NoInclusion) || valoracionActual.Motivacion_NoInclusion.Equals(" ")))
            {
                Error = string.Format("Indique la Motivación de la Valoración");
                ListItem li = new ListItem();
                li.Value = i.ToString();
                li.Text = Error;
                items.Add(li);
            ...
            if ((string.IsNullOrEmpty(valoracionActual.ResuelveArticulo1) || valoracionActual.ResuelveArticulo1.Equals(" "))
                && (string.IsNullOrEmpty(valoracionActual.ResuelveArticulo2) || valoracionActual.ResuelveArticulo2.Equals(" ")))
            {
                Error = string.Format("Indique la Resolución de los articulos");
                ListItem li = new ListItem();
                li.Value = i.ToString();
                li.Text = Error;
                items.Add(li);
    ...
    private void CargarInfoDeclaracion() {
        ...
        if (idValoracion != null) {
            ...
            clsDeclaracion declar = objGeneral.ObtenerDeclaracion(valoracionActual.DeclaracionId, llave);
            ...
            //Se recorren los hechos victimizantes para dependiendo del tipo de hecho mostrar siertas columas e información
            ...
            Session[ConstantesItems.HERRAMIENTAS] = herramien;
