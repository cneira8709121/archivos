...
public class LoginService : ILoginService
{
    ...
    public clsUsuario Authenticate(string cuenta, string contraseña, clsInterfaseRed ir, string info)
    {
        ...
        // Se cambia el uso de la variable info por la verificación de la ultima versión de RUV
        if (info == null)
        {
        ...
        {
            string ultimaVersion = System.Configuration.ConfigurationManager.AppSettings["UltimaVersion"];
            if (!info.Equals(ultimaVersion))
            {
                Usuario = new clsUsuario()
                {
                    MensajeAutenticacionFallida = "Autenticación fallida, el usuario no cuenta con la ultima versión de la captura"
                };
                return Usuario;
            }
        }
