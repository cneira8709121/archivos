...
public partial class PendientePublicacionEdicto : PaginaBase
{
    ...
    //    int nIdDeclaracion = (HttpContext.Current.Session[ConstantesNotificaciones.Notificaciones] as clsNotificacionDetalle).nIdDeclaracion;
    //    CargaDatosValoracionService valser = new CargaDatosValoracionService();
    //    int nIdValoracion = valser.GetIdValoracionByIdDeclaracion(nIdDeclaracion, ref cError);

    //    if (nIdValoracion != null)
    //    {
    ...
    protected void BtnDescargarFCA_Click(object sender, EventArgs e)
    {
        //string cError = string.Empty;
        //int nIdDeclaracion = (HttpContext.Current.Session[ConstantesNotificaciones.Notificaciones] as clsNotificacionDetalle).nIdDeclaracion;
        //CargaDatosValoracionService valser = new CargaDatosValoracionService();
    ...
    protected void btnSubirFCA_Click(object sender, EventArgs e)
    {
    ...
    protected void btnFinalizar_Click(object sender, EventArgs e)
    {
        ...
        else if (string.IsNullOrEmpty(txtObservacionNotificacion.Text))
        {
            ModalPopUp.MostrarMensaje("Advertencia", "Para poder finalizar debe cargar el formato de constancia de atención o escribir una observación");
        }
    ...
    protected void MarcarEdicto_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (fuCargarReporte.HasFile)
            {
                ...
                string fileName = "Edicto_" + Request.QueryString["id"] + Path.GetExtension(fuCargarReporte.FileName);
                fuCargarReporte.SaveAs(ConfigurationManager.AppSettings["PathArchivosNotificaciones"] + fileName); 
                
                int DiasHabiles = int.Parse(ConfigurationManager.AppSettings["PlazoPlanB"].ToString());
                //edicto publicado esta bien
                var Confirmar = service.ConfirmarPublicacionEdicto(nIdNotificacion, (int)eEstadosNotificacion.EdictoPublicado, DiasHabiles, txtObservacionNotificacion.Text, ref cError);
