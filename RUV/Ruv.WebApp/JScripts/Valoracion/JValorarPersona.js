...
function CambioEstado() {
    ...
    var ddlHechoEnmarcado = $('#ddlHechoEnmarcado');
    ...
    var hechoEnmarcado = ddlHechoEnmarcado.val();
    ...
    $('#ddlHechoEnmarcado option').each(function () {
        $(this).remove();
    });

    ddlHechoEnmarcado.append('<option value=0>[Seleccione Uno]</option>');

    $.each(HechoEnmarcado, function (index, hechoenmarcado) {
        if (estado == 1) {
            ddlHechoEnmarcado.append('<option value=' + hechoenmarcado.Id + '>' + hechoenmarcado.Nombre + '</option>');
        }
    });

    ddlHechoEnmarcado.val(hechoEnmarcado);
...
$(document).ready(function () {
    ...
    $.ajax({
        type: "POST",
        url: "Nueva.aspx/HechoEnmarcado",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (response) {
            HechoEnmarcado = response.d;
        },
        error: function (result) {
            alert('Ocurrio un error cargando los hechos enmarcados recargue la pagina para intentar de nuevo');
        }
    });
});