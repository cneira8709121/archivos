...
namespace Ruv.Infrastructure.Crosscutting.Common.Entidades
{
    public partial class clsAnexo13_Victima : clsEntidadBase, IDataErrorInfo
    {
                ...
                switch (columnName)
                {
                    ...
                    case "TipoDocumento":
                        ...
                        else
                        {
                          ...
                          {
                            resultado = ValidarPersonasRepetidasAnexo13(TipoDocumento, NumeroDocumento);                          
                          }
                    ...
                    case "Relacion":
                        ...
                        {                            
                            var PA = AnexoPadre.ListaPersonas;
        ...
        /// </summary>
        /// <returns></returns>
        string ValidarPersonasRepetidasAnexo13(int? TipoDocumento, string NumeroDocumento)
        {
          clsDeclaracion DA = clsDeclaracion.DeclaracionActual;
          string resultado = null;
          List<clsAnexo13_Victima> personaRepetida = new List<clsAnexo13_Victima>();

          //Se busca en la Hoja 2
          if (DA.PersonasAfectadas.ListaPersonas.Any(x => x.NumeroDocumento == NumeroDocumento && x.TipoDocumento == TipoDocumento))
            return  string.Format(
                        "El documento '{0}' con el numero '{1}' ya existe en la hoja 2 de esta declaración lo cual no esta permitido.",
          ...
          foreach (clsAnexo13 A13 in DA.A13)
          {
            ...
            if (A13.ListaPersonas.Any(x => x.NumeroDocumento == NumeroDocumento && x.TipoDocumento == TipoDocumento && x.ID != ID))
              return string.Format(
                          "El documento '{0}' con el numero '{1}' ya existe en otro anexo 13 de esta declaración lo cual no esta permitido.",
                          Enum.GetName(typeof(eTipoDocumento), TipoDocumento), NumeroDocumento);            
          }
