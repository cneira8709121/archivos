...
{
    public class Anexo06
    {
        public const int TipoAnexo = (int)Common.eTiposAnexo.Anexo06;

        #region Guardar
        public static void Guardar(clsAnexo06 anexoSiniestro, int idValoracion, DbTransaction tran)
        {
            int idValanexo = 0;
            Anexo06.GuardarSiniestro(anexoSiniestro, idValoracion, ref idValanexo, tran);

            //Obtener Anexo
            if (idValanexo == 0)
            {
                Anexo06.ObtenerValoracionAnexo(anexoSiniestro, idValoracion, ref idValanexo, tran);
            }
            ...
            //TBANEXO06
            foreach (clsAnexo06_Victima anexoView in anexoSiniestro.Victimas)
            {
                //Guardar Anexo
                Anexo06.GuardarAnexo(anexoView, id_siniestro, idValanexo, tran);
        ...
        public static void ObtenerValoracionAnexo(clsAnexo06 anexoSiniestro, int idValoracion, ref int idValanexo, DbTransaction tran)
        {
        ...
        #region Siniestro
        private static void GuardarSiniestro(clsAnexo06 anexoSiniestro, int idValoracion, ref int idValanexo, DbTransaction tran)
        {
            entSiniestroPersona entBdSiniestro = new entSiniestroPersona();

            TBSINIESTROS_PERSONA siniestroData = new TBSINIESTROS_PERSONA();
            Anexo06.ParseViewToData_Siniestro(anexoSiniestro, ref siniestroData);

            //Asignar tipo hecho
            siniestroData.PARAM_TIPOHECHO = (int)Common.eTiposAnexo.Anexo06;
        ...
        public static void ParseViewToData_Siniestro(clsAnexo06 anexo, ref Ruv.Data.TBSINIESTROS_PERSONA siniestroData)
        {
        ...
        #region Anexo
        private static void GuardarAnexo(clsAnexo06_Victima anexoView, int id_siniestro, int idValanexo, DbTransaction tran)
        {
            TBANEXO6 anexoData = new TBANEXO6();
            Anexo06.ParseViewToData_Anexo(anexoView, id_siniestro, ref anexoData);
            entAnexo6 entBd = new entAnexo6();
            entSiniestroPersona entBdSiniestro = new entSiniestroPersona();
            ...
            switch (anexoView.EstadoRegistro)
            {
                case eEstadoRegistro.Insertar:
                    //Activar
                    anexoData.ACTIVO = 1;
                    entBd.setAnexo6(anexoData, tran);
                    ...
                    break;
                case eEstadoRegistro.Modificado:
                    entBd.updAnexo6(anexoData, tran);
                    break;
                case eEstadoRegistro.Eliminado:
                    //Desactivar
                    anexoData.ACTIVO = 0;
        ...
        public static void ParseViewToData_Anexo(clsAnexo06_Victima anexoView,  int id_siniestro, ref TBANEXO6 anexoData)
        {
            #region Common Anexos
            anexoData.ID = anexoView.ID ?? -1;

            anexoData.FALLECIDA = Common.ParseIntToShortNullable(anexoView.VictimaFallecida);
            ...
            #region Datos del Homicidio o Masacre
            anexoData.QUEDO_ALGUN_HUERFANO  = Ruv.Business.Captura.Common.ParseIntToShortNullable(anexoView.AlgunMenorQuedoHuerfano);
            anexoData.ID_HUERFANO = anexoView.MenorDesprotegidoId;
            anexoData.PARAM_HUERFANO_DE = anexoView.MenorQuedoHuerfanoDe;
            anexoData.NUM_PERSONAS_MUERTAS  = Ruv.Business.Captura.Common.ParseIntToShortNullable(anexoView.NumeroPersonasMuertasEnEsteHecho);
            #endregion
        ...
        public static clsAnexo06 ObtenerAnexo(TBSINIESTROS_PERSONA siniestroData)
        {
            entAnexo6 entAnexo = new entAnexo6();
            clsAnexo06 anexoView = new clsAnexo06();

            //Pasar datos a la vista
            Anexo06.ParseDataToView_Siniestro(siniestroData, ref anexoView);

            //Reiniciar Estado
            anexoView.EstadoRegistro = eEstadoRegistro.SinModificaciones;

            //Obtener las personas del anexo
            List<TBANEXO6> anexosData = entAnexo.getData(siniestroData.ID);

            foreach (TBANEXO6 anexoData in anexosData)
            {
                clsAnexo06_Victima VictimaAnexoView = new clsAnexo06_Victima();

                Anexo06.ParseDataToView_Anexo(anexoData, VictimaAnexoView);
        ...
        public static void ParseDataToView_Siniestro(Ruv.Data.TBSINIESTROS_PERSONA siniestroData, ref clsAnexo06 anexoView)
        {
        ...
        public static void ParseDataToView_Anexo(TBANEXO6 anexoData, clsAnexo06_Victima anexoView)
        {
            anexoView.ID = anexoData.ID;
            anexoView.VictimaFallecida = anexoData.FALLECIDA;
            ...
            //A10
            clsAnexo_Afectacion anexoAfectacionView = anexoView.Afectacion;
            anexoAfectacionView.Afectado = anexoData.AFECTADO;
            anexoAfectacionView.Otro = anexoData.OTRA_AFECTACION;
            anexoAfectacionView.AfectadoId = anexoData.TBREGISTROS_PERSONAS.ID;
            anexoView.Afectacion = anexoAfectacionView;
            ...
            anexoView.DenunciaPrevia = denunciaPreviaView;
            anexoView.MenorQuedoHuerfanoDe = anexoData.PARAM_HUERFANO_DE;
            anexoView.MenorDesprotegidoId = anexoData.ID_HUERFANO;

            anexoView.NumeroPersonasMuertasEnEsteHecho = anexoData.NUM_PERSONAS_MUERTAS;

            anexoView.RecuerdaNumeroPersonasMuertas = (anexoView.NumeroPersonasMuertasEnEsteHecho == null || anexoView.NumeroPersonasMuertasEnEsteHecho < 1) ? 0 : 1;
